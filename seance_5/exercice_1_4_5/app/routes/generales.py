...
def un_pays(nom_pays):
    try:
        pays = Country.query.filter(Country.name == nom_pays).first()
        if pays is None:
            flash(f"Le pays '{nom_pays}' n'a pas été trouvé.", "warning")
            return render_template('erreur/404.html', sous_titre="Pays non trouvée"), 404
        ...
        flash(f"Une erreur est inattendue est survenue")
...
#-------------------------------exericie 4-----------------------------------------
@app.route("/recherche_instant", methods=['GET'])
def recherche_instant():
    chaine = request.args.get("chaine", None)

    if chaine:
        resultats = [country.name for country in Country.query.filter(Country.name.ilike(f"%{chaine}%")).all()]
        return jsonify(resultats)

#Prend en charge la navigation dans les pages et fournit des données JSON pour des suggestions de recherche en temps réel
#----------------------- fin exercice 4 -------------------------------------
